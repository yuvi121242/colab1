...
const STORAGE_KEY = 'colab_v2_app_state';
const PERMISSIONS_KEY = 'colab_v2_permissions_done';
...
// ============================================
// INJECTED JS - Minimal, no auth popup hacks
// Auth is handled NATIVELY via the patch
// ============================================
...
(function() {
  ...
  // Auto-accept navigation confirmations
  ...
  // Lock Page Visibility API
  try {
    Object.defineProperty(document, 'hidden', { get: function() { return false; }, configurable: true });
    Object.defineProperty(document, 'visibilityState', { get: function() { return 'visible'; }, configurable: true });
  } catch(e) {}

  // Block visibility/blur/pagehide/freeze events
...
export default function ColabApp() {
  ...
  // ============================================
  // ANTI-IDLE - Lightweight poke every 45s
  // ============================================
  useEffect(() => {
  ...
  // ============================================
  // INJECTED JS - Anti-idle + viewport
  // ============================================
  const getInjectedScript = () => `
    (function() {
      ...
      if (${antiIdle}) {
        ...
        // Block visibility/blur events (one-time)
    ...
    <View style={styles.container}>
      ...
      {isLoading && (
        <View style={styles.progressBar}>
          <View style={[styles.progressFill, { width: `${progress * 100}%`, backgroundColor: APP_COLOR }]} />
        </View>
      )}
        ...
        {tabs.map(tab => (
          ...
          >
            <WebView
              ...
              // CRITICAL: Enable native popup support
              // Auth popups are handled NATIVELY via the patch (Dialog + WebViewTransport)
              // No JS-level popup handling needed
      ...
      {showNavBar && (
        <View style={[styles.navBar, { paddingBottom: bottomPadding }]}>
          ...
          <TouchableOpacity style={[styles.navBtn, !canGoForward && styles.navBtnDisabled]} onPress={goForward} disabled={!canGoForward}>
            <Ionicons name="chevron-forward" size={24} color={canGoForward ? "#fff" : "#555"} />
          </TouchableOpacity>
          <TouchableOpacity style={styles.navBtn} onPress={goHome}>
            <Ionicons name="home" size={22} color="#fff" />
          </TouchableOpacity>
          <TouchableOpacity style={styles.navBtn} onPress={reload}>
            <Ionicons name="refresh" size={22} color="#fff" />
          </TouchableOpacity>
          <TouchableOpacity style={[styles.navBtn, desktopMode && styles.navBtnActive]} onPress={() => { setDesktopMode(!desktopMode); reload(); }}>
            <Ionicons name={desktopMode ? "desktop" : "phone-portrait"} size={22} color={desktopMode ? APP_COLOR : "#fff"} />
          </TouchableOpacity>
          <TouchableOpacity style={styles.navBtn} onPress={() => setShowNavBar(false)}>
      ...
      {/* FAB - Quick Actions */}
      <TouchableOpacity
        style={[styles.fab, { backgroundColor: APP_COLOR, bottom: showNavBar ? bottomPadding + 80 : bottomPadding + 20 }]}
      ...
      <Modal visible={showQuickActions} transparent animationType="fade">
        <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={() => setShowQuickActions(false)}>
          <View style={[styles.quickMenu, { bottom: showNavBar ? bottomPadding + 140 : bottomPadding + 80 }]}>
            <TouchableOpacity style={styles.qItem} onPress={() => { saveBookmark(); setShowQuickActions(false); }}>
              <Ionicons name="bookmark" size={24} color="#F9AB00" />
              <Text style={styles.qText}>Save Bookmark</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.qItem} onPress={() => { setShowBookmarks(true); setShowQuickActions(false); }}>
              <Ionicons name="bookmarks" size={24} color="#20BEFF" />
              <Text style={styles.qText}>View Bookmarks</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.qItem} onPress={() => { zoomIn(); setShowQuickActions(false); }}>
              <Ionicons name="add-circle" size={24} color="#4CAF50" />
              <Text style={styles.qText}>Zoom In ({zoomLevel}%)</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.qItem} onPress={() => { zoomOut(); setShowQuickActions(false); }}>
              <Ionicons name="remove-circle" size={24} color="#FF5722" />
              <Text style={styles.qText}>Zoom Out</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.qItem} onPress={() => { resetZoom(); setShowQuickActions(false); }}>
              <Ionicons name="resize" size={24} color="#9C27B0" />
              <Text style={styles.qText}>Reset Zoom</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.qItem} onPress={() => { Linking.openSettings(); setShowQuickActions(false); }}>
              <Ionicons name="battery-charging" size={24} color="#4CAF50" />
              <Text style={styles.qText}>Battery Settings</Text>
            </TouchableOpacity>
            {!showNavBar && (
              <TouchableOpacity style={styles.qItem} onPress={() => { setShowNavBar(true); setShowQuickActions(false); }}>
                <Ionicons name="chevron-up" size={24} color="#fff" />
                <Text style={styles.qText}>Show Nav Bar</Text>
              </TouchableOpacity>
            )}
          </View>
        </TouchableOpacity>
      ...
      <Modal visible={showPermSetup} animationType="slide" transparent>
        <View style={styles.overlay}>
          <View style={styles.permModal}>
            ...
            <TouchableOpacity style={[styles.permDoneBtn, { backgroundColor: APP_COLOR }]} onPress={finishPermSetup}>
              <Text style={styles.permDoneBtnText}>
                {permBattery && permNotif ? "All Done!" : "Continue Anyway"}
              </Text>
            </TouchableOpacity>

            <Text style={styles.permNote}>You can change these later in your phone's Settings app.</Text>
          </View>
        </View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  bmUrl: { color: '#888', fontSize: 11, marginTop: 2 },

  // Web Fallback
  webFallback: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 40 },
  title: { color: '#fff', fontSize: 32, fontWeight: '700', marginTop: 20 },
  subtitle: { color: '#888', fontSize: 16, marginTop: 8, textAlign: 'center' },
  btn: { paddingHorizontal: 24, paddingVertical: 14, borderRadius: 12, marginTop: 24 },
  btnText: { color: '#fff', fontSize: 16, fontWeight: '600' },

  // Permission Modal
  permModal: { width: '90%', backgroundColor: '#2d2d44', borderRadius: 20, padding: 24, maxHeight: '85%' },
  permCloseBtn: { position: 'absolute', top: 16, right: 16, zIndex: 10, width: 36, height: 36, borderRadius: 18, backgroundColor: '#3d3d54', justifyContent: 'center', alignItems: 'center' },
  permTitle: { color: '#fff', fontSize: 20, fontWeight: '700', marginTop: 12, marginBottom: 4 },
  permDesc: { color: '#aaa', fontSize: 14, textAlign: 'center', paddingHorizontal: 8 },
  permItem: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#3d3d54', borderRadius: 14, padding: 16, marginVertical: 8 },
  ...
  permItemDesc: { color: '#aaa', fontSize: 12, marginTop: 2 },
  permDoneBtn: { borderRadius: 14, padding: 16, alignItems: 'center', marginTop: 16 },
  permDoneBtnText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  permNote: { color: '#888', fontSize: 12, textAlign: 'center', marginTop: 12 },
});
